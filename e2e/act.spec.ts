...
test("опись-акт (Доставка в аренду, без расценки): акт прикладывается, ведомость не появляется", async ({
  browser,
}) => {
  test.slow();
  const mctx = await browser.newContext();
...
test("карточка диспетчера: секция «Акт» — от «ожидается» к «приложен»; PDF открывается ссылкой", async ({
  ...
  const milena = await mctx.newPage();
  await login(milena, "milena");
  const id = await createAssignedTask(milena, "Алексей Каширский", "Забор в ремонт"); // акт-опись, без расценки
